...
- (id)initWithFrame:(CGRect)frame scrollView:(UIScrollView *)scrollView
{
    ...
    if (self) {
        self.progress = 0;
        self.refreshItems = [NSMutableArray array];
...
- (void)setupItems {
    ...
    [self addRefreshItem:[[RWTRefreshItem alloc] initWithView:self.buildings centerEnd:CGPointMake(CGRectGetMidX(self.bounds), CGRectGetHeight(self.bounds) - CGRectGetHeight(self.ground.bounds) - CGRectGetHeight(self.buildings.bounds)/2) parallaxRatio:1.5f sceneHeight:kSceneHeight]];
    ...
    [self addRefreshItem:[[RWTRefreshItem alloc] initWithView:self.cat centerEnd:CGPointMake(CGRectGetMidX(self.bounds), CGRectGetHeight(self.bounds) - CGRectGetHeight(self.ground.bounds)/2 - CGRectGetHeight(self.cat.bounds)/2) parallaxRatio:1.0f sceneHeight:kSceneHeight]];
...
- (void)refreshColor {
    CGFloat value = (0.7*self.progress) + 0.2;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    self.progress = MIN(MAX(offsetY / kSceneHeight, 0.f), 1.f);
    [self refreshColor];
...
- (void)beginRefreshing {
    ...
    [UIView animateWithDuration:0.2 delay:0 options:
     UIViewAnimationOptionRepeat|UIViewAnimationOptionAutoreverse
                     animations:^{
                         self.capeBack.transform =
                         CGAffineTransformMakeRotation(M_PI/32);
                         self.cat.transform = CGAffineTransformMakeTranslation(-1.0, 0);
                     } completion:^(BOOL finished) {
                     }];
    // Move ground
    [UIView animateWithDuration:1.f delay:0.25
                        options:UIViewAnimationOptionCurveEaseIn animations:^{
                            self.ground.center = CGPointMake(self.ground.center.x,
                                                             self.ground.center.y + kSceneHeight);
                            self.buildings.center = CGPointMake(self.buildings.center.x,
                                                                self.buildings.center.y + kSceneHeight);
                        } completion:^(BOOL finished) {
                        }];
